...
object KafkaConsumer {
  ...
  object Conf {
    ...
    def apply[K, V](
      ...
      val configMap = Map[String, AnyRef](
        ...
        ConsumerConfig.ISOLATION_LEVEL_CONFIG -> isolationLevel.toString
          .toLowerCase()
      )
  ...
  final case class Conf[K, V](
    ...
    def isAutoCommitMode: Boolean =
      props
        .getOrElse(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "")
        .toString
        .equals("true")
  ...
  def apply[K, V](conf: Conf[K, V]): JKafkaConsumer[K, V] = {
    ...
    new JKafkaConsumer[K, V](
      conf.props.asJava,
      conf.keyDeserializer,
      conf.valueDeserializer
    )
  }
}
